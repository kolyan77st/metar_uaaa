...
def build_prompt(metar_raw: str, taf_raw: str) -> str:
    prompt = f\"\"\"Ты — авиационный метеоролог. На основе METAR и TAF составь краткий и понятный прогноз для пилота на ближайшие 6 часов.
Ответ должен содержать пункты: 1) Ветер — изменения и порывы, 2) Видимость — улучшение/ухудшение, 3) Облачность/потолок, 4) Опасные явления (туман, ливни, грозы и т.д.), и краткий итог (1-2 предложения).
METAR: {metar_raw}
TAF: {taf_raw}
Пиши коротко, по пунктам, без лишних слов.
\"\"\"
    return prompt
